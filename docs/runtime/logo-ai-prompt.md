# VoxLens Logo AI Prompt

Use this prompt in Midjourney, Ideogram, Recraft, GPT image tools or other AI image products. The current project already includes a deterministic SVG implementation at `apps/web/public/brand/voxlens-mark.svg`; this prompt explores the same direction while staying aligned with the app's soft aurora/glass interface.
...
Color palette: paper white #FBFDFF, ink plum #201A36, aurora sky #79D7FF, prism violet #C28DFF, creator blush #FF9FCF, tiny citation coral #FF667C. Avoid dark purple dominance and avoid ocean/water-drop styling.
Materials/textures: translucent glass cards, soft gradients, crisp vector edges, subtle depth, no photorealism, no 3D mascot.
Constraints: original, clean at 32px, recognizable in monochrome, suitable for web app favicon, GitHub README and product landing page.
Avoid: water-drop logo, ocean wave logo, generic magnifying glass, robot face, social media platform logos, busy network graph, dark cyberpunk style, excessive AI sparkles, fake Chinese characters, unreadable micro details, watermark, mockup background.
```

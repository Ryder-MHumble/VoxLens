"""MediaCrawler subprocess provider."""
...
from social_deep_research.utils import ROOT, read_jsonl_files, run_command, slugify


MEDIACRAWLER_PLATFORM = {
    "bilibili": "bili",
    "xiaohongshu": "xhs",
    "douyin": "dy",
    "kuaishou": "ks",
...
def supports(platform: str) -> bool:
    return platform in MEDIACRAWLER_PLATFORM
...
def search(
    ...
    platform_code = MEDIACRAWLER_PLATFORM.get(platform)
    if not platform_code:
        return ProviderRun(provider="mediacrawler", platform=platform, query=query, ok=False, note="MediaCrawler does not support this platform")

    media_crawler_dir = media_crawler_dir or ROOT / "external" / "MediaCrawler"
    save_root = save_root or ROOT / "runs" / "mediacrawler" / slugify(query)
    run = ProviderRun(provider="mediacrawler", platform=platform, query=query)
    if not (media_crawler_dir / "main.py").exists():
        run.note = f"MediaCrawler not found at {media_crawler_dir}"
        return run
    ...
    run.command = command
    try:
        code, stdout, stderr, elapsed = run_command(command, cwd=media_crawler_dir, timeout=timeout)

...
> 面向视频社媒证据的 DeepResearch alpha：输入一个研究问题，VoxLens 会跨 Bilibili、Douyin、YouTube、小红书/RedNote、知乎、快手、微博等来源搜索内容，优先采样标题、元数据、评论、字幕/逐字稿或文本片段，并生成带证据引用和质量评估的研究报告。
...
VoxLens 是一个证据优先的视频社媒研究产品，帮助产品、市场、运营、内容和研究团队，把分散在标题、简介、评论、字幕/逐字稿、创作者内容和平台元数据里的信号，转化为可引用、可复盘、可行动的业务洞察。

VoxLens 的目标不是“多抓数据”，也不是暗示已经逐帧理解每条视频；它优先把可访问的视频社媒证据变成结构化、可引用、可评估的研究报告。视频画面、音频和关键帧理解可以作为后续增强，但当前 alpha 以字幕、评论和上下文证据为主。
...
- 信息分散在不同平台、不同视频和评论区中。
- 视频研究通常先依赖标题、简介、字幕/逐字稿、评论和平台上下文；关键帧或音频理解是增强能力，不是当前 alpha 的默认前提。
- 评论区噪声高，但包含大量一线用户语言。
- 研究结论经常缺少可回溯的证据来源。
...
- **证据优先采样**：采集视频标题、基础信息、评论区样本、字幕/逐字稿或文本片段，用于后续证据归因；视频画面/音频理解作为后续增强，不作为当前能力承诺。
...
| 证据粒度 | 通常引用网页或搜索结果 | 更关注评论、字幕/逐字稿、文本片段、来源和帖子级证据 |
| 面向用户 | 技术用户、研究人员、自动化使用者 | 产品、市场、运营、内容、研究和业务团队 |

VoxLens 不追求替代通用 Agent。它更像一个视频社媒证据研究助手：帮你找到相关来源，抽样可访问的评论、字幕/逐字稿和上下文，整理证据，生成业务可读的报告，并在证据不足时标出结论风险。

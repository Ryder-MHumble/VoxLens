# VoxLens Studio — 从视频里拿证据，不是拿答案。

<p align="center">
  <a href="README.md">English</a> · <a href="README.zh-CN.md">中文</a>
</p>

> 证据优先的中文视频研究工作台：提出研究问题，在你可控的环境中采集可访问证据，抽取口播、字幕、评论和上下文信号，再生成可审计、带引用并明确质量边界的研究报告。
...
**VoxLens Studio 在你的环境里，把中文视频口播、字幕、评论和元数据转成可审计、可引用的研究证据。** 它面向产品、市场、运营、内容和研究团队，而不是一个承诺替用户抓遍平台的通用答案引擎。

产品不承诺全平台稳定覆盖，也不暗示已经逐帧理解每条视频。采集依赖用户授权的本地环境或 provider；当前 alpha 以字幕、评论和上下文证据为主，ASR 执行层仍通过适配器接入。覆盖、来源和 grounding 检查可以阻止 LLM 综合，并降级为确定性的证据受限报告。
...
VoxLens Studio 不追求替代通用 Agent。它是一个视频证据研究工作台：找到来源、保存 artifact、抽取可复核 EvidenceUnit、验证报告 Claim，并在证据不足时标出结论风险。

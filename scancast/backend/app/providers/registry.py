...
from app.providers.mediacrawler_provider import (
    search_bilibili_mediacrawler,
    search_douyin,
    search_kuaishou,
    search_weibo,
    search_xiaohongshu,
...
class LocalDevProvider:
    ...
    def search(self, request: ResearchRequest, target: CrawlTarget) -> tuple[list[Source], RunLog]:
        ...
        if target.platform == "bilibili" and target.provider == "mediacrawler":
            return search_bilibili_mediacrawler(
                target.query,
                target.limit,
                target.commentsLimit,
                request.useMediaCrawler,
                target.authMode,
                target.videoParallelism,
            )
        if target.platform == "douyin" and target.provider == "mediacrawler":
        ...
        if target.platform == "xiaohongshu" and target.provider == "mediacrawler":
        ...
        if target.platform == "zhihu" and target.provider == "mediacrawler":
            return search_zhihu(
                target.query,
                target.limit,
                target.commentsLimit,
        ...
        if target.platform == "kuaishou" and target.provider == "mediacrawler":
        ...
        if target.platform == "weibo" and target.provider == "mediacrawler":
            return search_weibo(
                target.query,
                target.limit,
                target.commentsLimit,
...
def provider_capabilities() -> dict[str, Any]:
    ...
    return {
        ...
        "defaultMode": "local",
        "localDevProviders": ["opencli", "mediacrawler", "yt-dlp"],
        "onlineProvider": {

    ...
    if not sources:
        return [
            ...
            ReportSection(
                ...
                kind="next_actions",
                body=(
                    "建议先确认 MediaCrawler/CDP 是否可用；如涉及 Bilibili、Douyin、小红书或知乎，请配置 Cookie 或使用已有浏览器登录态后重试。"
                    if zh
                    else "Check OpenCLI/MediaCrawler availability first; for Bilibili, Douyin, Xiaohongshu or Zhihu, configure cookies or reuse a logged-in browser/CDP session, then retry."
                ),
            ),
        ]

...
def build_research_plan(request: ResearchRequest) -> tuple[ResearchPlan, AgentStep]:
    ...
    for platform in request.platforms:
        primary = _primary_provider(platform, request.useMediaCrawler)
        ...
        if platform == "bilibili" and request.useMediaCrawler:
            targets.append(CrawlTarget(
                platform=platform,
                provider="opencli" if primary == "mediacrawler" else "mediacrawler",
                query=_platform_query(platform, expanded),
                limit=request.limitPerPlatform,
                commentsLimit=request.commentsPerVideo,
                detailLimit=request.detailVideosPerPlatform,
    ...
    plan = ResearchPlan(
        ...
        notes=[
            ...
            "MediaCrawler is primary for Chinese social platforms and uses cookie env vars or existing-browser/CDP login state.",
...
def _primary_provider(platform: str, use_media_crawler: bool) -> str:
    if use_media_crawler and platform in {"bilibili", "douyin", "xiaohongshu", "zhihu", "kuaishou", "weibo"}:
        return "mediacrawler"
    return "opencli"
